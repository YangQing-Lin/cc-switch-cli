...
#### Codex 支持
- [ ] Codex 配置管理
- [ ] 原子写入 `~/.codex/auth.json` + `~/.codex/config.toml`
- [ ] 双文件事务机制（失败自动回滚）
- [ ] TOML 语法校验
- [ ] Codex 配置验证（必须包含 auth 字段）
...
- [ ] 按名称+API Key去重
- [ ] 迁移后归档到 `~/.cc-switch/archive/<timestamp>/`
- [ ] 创建迁移标记文件 `~/.cc-switch/migrated.copies.v1`
...
- [ ] 自动检测 VS Code settings.json 路径
- [ ] 读取 VS Code 设置
- [ ] 写入 VS Code 设置（保留注释格式）
- [ ] 支持多个 VS Code 版本（Stable、Insiders、VSCodium、OSS）

### 6. 应用设置

- [ ] 读取应用设置 `~/.cc-switch/settings.json`
- [ ] 保存应用设置
- [ ] 语言切换支持（en/zh）
- [ ] 自定义配置目录设置
